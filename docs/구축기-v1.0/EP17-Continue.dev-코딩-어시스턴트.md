...
> 시리즈: [폐쇄망 LLM 구축기](README.md) · 이전: [EP16. 폐쇄망 RAG 구축 ②](EP16-폐쇄망-RAG-구축-2.md) · 다음: [EP18. 시리즈 총정리 & 운영 회고](EP18-시리즈-총정리-운영-회고.md)

EP01부터 계속 "현업팀 QA봇 + 개발팀 코딩 어시스턴트" 두 갈래라고 말씀드렸었는데, 드디어 개발팀 쪽 얘기예요. [[Continue.dev]]로 VS Code에 "사내판 Copilot"을 붙이는 편입니다.

## 📦 VS Code 확장부터 오프라인으로 깔았어요

Continue.dev는 보통 VS Code 마켓플레이스에서 클릭 한 번으로 설치하는데, 마켓플레이스 자체가 인터넷 서비스라 개발자 PC(폐쇄망 안)에서는 그 화면 자체가 안 열려요. EP04에서 미리 받아둔 `.vsix` 파일을 오프라인으로 설치했습니다.

```bash
code --install-extension ./import/extensions/continue.vsix
```

이거 하나만 알면 되는데, 처음엔 이 사실 자체를 몰라서 "확장 마켓플레이스가 왜 안 열리지"로 한참 헤맸어요.
...
📌 **EP.17 한 줄 요약**
VS Code 확장은 마켓플레이스 대신 EP04에서 받아둔 `.vsix`로 오프라인 설치했다. 자동완성용 소형 코드 모델(1.5B)을 위해 32B 모델의 VRAM 점유율을 살짝 낮추고 남는 공간에 세 번째 vLLM 인스턴스를 얹었다. Continue.dev도 Open WebUI와 마찬가지로 LiteLLM 게이트웨이를 거치게 해 팀별 예산 관리 안에 뒀다.

**다음 편**: [EP18. 시리즈 총정리 & 운영 회고](EP18-시리즈-총정리-운영-회고.md)
